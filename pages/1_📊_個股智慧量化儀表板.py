...
if user_input:
  ...
  if target_code:
    ...
    try:
      with st.spinner(
          f"⏳ 正在同步 【{target_code} {stock_name}】 {selected_days}"
          " 天歷史與量化滿足點計算..."
      ):
        ...
        stock = twstock.Stock(target_code)
        if len(stock.price) < 30:
          st.error("❌ 歷史資料不足。")
        else:
          prices = [float(p) for p in stock.price]
          ...
          p_latest, p_prev = prices[-1], prices[-2]
          chg_latest = round(p_latest - p_prev, 2)
          pct_latest = round(((p_latest - p_prev) / p_prev) * 100, 2)
          ...
          st.success(
              f"📅 **目前個股收盤資料日期：{df_chart['Date_Str'].iloc[-1]}** (已載入"
              f" {selected_days} 天長天期歷史)"
          )
          ...
          df_inst_api, df_chips_15 = fetch_finmind_chips(target_code)
          ...
          macd_today, macd_yesterday = float(df_chart["MACD_Bar"].iloc[-1]), (
              float(df_chart["MACD_Bar"].iloc[-2])
          )
          is_green_to_red = (macd_yesterday < 0) and (macd_today >= 0)
          ...
          with tab1:
            ...
            st.markdown(
                "您可以直接在下方輸入您的狀況（例如：*「我均價在 134.18，明天如果開平盤量縮我該怎麼掛單？」*），按下"
                " **Enter 鍵** 或點擊按鈕即可發送！"
            )
